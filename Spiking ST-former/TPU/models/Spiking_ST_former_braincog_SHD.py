...
class STSA(BaseModule):
    def __init__(self,dim,step=10,encode_type='direct',num_heads=16,TPU_Belta=0.5,qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
...
class Block(nn.Module):
    def __init__(self, dim, num_heads, step=10,TPU_Belta=0.5, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., norm_layer=nn.LayerNorm, sr_ratio=1):
        super().__init__()
        ...
        self.attn = SSA(dim, step=step,TPU_Belta=TPU_Belta,num_heads=num_heads, qkv_bias=qkv_bias, qk_scale=qk_scale,
...
class Spiking_ST_former(nn.Module):
    def __init__(self, step=10,TPU_Belta=0.5,if_UCF=False,
        ...
        block = nn.ModuleList([Block(step=step, TPU_Belta=TPU_Belta,
...
@register_model
def Spiking_ST_former_shd(pretrained=False, **kwargs):
    model = Spikformer(TPU_Belta=0.5,step=10,if_UCF=False,num_classes=20,
