...
class STSA(nn.Module):
    def __init__(self, dim, num_heads=8, TIM_alpha=0.5,qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,):
        super().__init__()
        ...
        self.TIM = TIM(TIM_alpha=TIM_alpha, in_channels=64)  # 插入模块
    ...
    def forward(self, x):
        ...
        T, B, C, N = x.shape #[T, B, C, N]
        x_for_qkv = x.flatten(0, 1)#[T*B, C, H*W]


        q_conv_out = self.q_conv(x_for_qkv)#[T*B, C, H*W]
        q_conv_out = self.q_bn(q_conv_out).reshape(T, B, C, N)#[T,B, C, H*W]
        q_conv_out = self.q_lif(q_conv_out)#[T,B, C, H*W]
        ...
        q = self.TIM(q)  #TIM input [T, B, H, N, C/H]
        # print(q.shape)
...
# T, B, C, H, W, = 4, 64,256,16,16
# x = torch.randn(T, B, C, H, W)
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
# model = SpikingSelfAttention(dim=256)
# model.to(device)  # Move the model to the specified device
# x = x.to(device)  # Move the input tensor to the specified device
# model(x)  # Call the forward method of the model with the input tensor x
...
@register_model
def Spiking_ST_former(pretrained=False, **kwargs):
    model = vit_snn(
